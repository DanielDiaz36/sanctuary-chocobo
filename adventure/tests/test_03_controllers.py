...
from .test_02_usecases import MockJourneyRepository, MockNotifier
...
class TestStopJourneyAPIView:
    ...
    def test_stop_ok(self, client, mocker):
        ...
        mocker.patch.object(
            views.EndJourneyAPIView,
            "get_repository",
            return_value=JourneyRepository(),
        )

        response = client.put(f"/api/adventure/end/{journey.id}/")

        assert response.status_code == status.HTTP_200_OK

    @pytest.mark.django_db
    def test_stop_fail(self, client, mocker):
        wrong_journey_id = 99999
        response = client.put(f"/api/adventure/end/{wrong_journey_id}/")

        assert response.status_code == 400
        assert response.json().get('detail') == 'Journey matching query does not exist.'
